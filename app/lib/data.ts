...
export async function fetchFilteredInvoices(
  ...
  try {
    ...
    // `;
    //use supabase api to fetch filtered invoices with

    console.log('Fetching filtered invoices with query:', ITEMS_PER_PAGE, 'and page:', currentPage);
    const invoicesSearch = await supabase.rpc('search_invoices', {
      _query: query,
    ...
    console.log('Fetched filtered invoices:', invoicesSearch);

    const invoices: InvoicesTable[] = invoicesSearch.data?.map((invoice: any) => ({
      id: invoice.id,
      amount: invoice.amount,
      date: invoice.date,
...
export async function fetchInvoicesPages(query: string) {
  try {
    ...
    const { data, error } = await supabase
      .rpc('count_search_invoices', {
        _query: query

      })
    if (error) console.error(error)
    else console.log('Fetched total invoices count for pagination:', data);

    const InvoicesTotalNumber = data ?? 0;

    const totalPages = Math.ceil(Number(InvoicesTotalNumber / ITEMS_PER_PAGE));
    return totalPages;
  } catch (error) {
    console.error('Database Error:', error);
